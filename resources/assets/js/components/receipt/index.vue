...
<ul class="customUl">
<li style="background:#41b883;margin-bottom:0px;padding:.5em;" class="hvr-bounce-to-right" v-for="v in value">

			<p style="color:#fff;" v-if="admissions.includes(v.title)">{{v.title}} + Admission</p>

			<p style="color:#fff;" v-else>{{v.title}}</p>
</li>
  </ul>
</div>
														<div class="col-md-8">
										...
										<div class="row">
											...
											<div class="col-md-12">
													<div class="form-group">
															...
															<div class="radio-custom radio-inline radio-success">
																<input type="radio" value="cheque" v-model="payment" name="payment">
...
<receipt-table v-if="this.$store.state.admin == 1"></receipt-table>
  ...
  export default {
    ...
    methods: {
    	...
    	resetStates(){
    		...
    		this.cf1value5 = ''
    		this.upHere  = false
    		this.vuemultiselectvalid  = false
    		this.onoff  = []
    		this.admissions  = []
    		this.activeToggle = []
			this.Member = ''
			...
			this.Check = ''
			this.Branch = ''
			this.Instructions = 'Additional Notes For Member Here'
			this.checkvalidated = 0
			this.hidesubmit = true
    ...
    mounted() {
		...
		// EventBus.$on('proceeded-to-print', clickCount => {
		//   this.resetStates()
		// })
